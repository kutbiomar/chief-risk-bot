...
from ..models.auth import ApiKey, AuthChallenge, PasswordResetToken, UserSession
...
def get_me(auth: AuthContext = Depends(require_session), db: Session = Depends(get_db)) -> SessionResponse:
    _, user = auth
    return SessionResponse(user=_serialize_user(db, user))
...
def reset_password(payload: ResetPasswordRequest, db: Session = Depends(get_db)) -> MessageResponse:
    reset_token = db.scalar(
        select(PasswordResetToken).where(PasswordResetToken.token_hash == sha256_hex(payload.token))
    )
    if (
        reset_token is None
    ...
    db.execute(
        update(UserSession).where(UserSession.user_id == user.id, UserSession.revoked_at.is_(None)).values(revoked_at=utc_now())
    )
    db.execute(
        update(AuthChallenge).where(AuthChallenge.user_id == user.id, AuthChallenge.consumed_at.is_(None)).values(consumed_at=utc_now())
    )
    db.commit()
