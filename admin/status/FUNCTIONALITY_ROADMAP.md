...
| RBAC | Partially implemented; workspace-admin settings require owner/admin, portfolio and document mutations require analyst/admin/owner | `backend/models/auth.py`, `backend/routers/auth.py`, `backend/routers/settings.py`, `backend/routers/portfolio.py`, `backend/routers/documents.py` |
...
2. **Enforce RBAC**
   - Shared role dependency, workspace-admin settings guards, manual portfolio mutation guards, and document mutation guards are in place.
   - Continue gating briefing publish and overlay/risk runs.
...
| Role authorization | `backend/routers/*.py` | Settings/API-key/member, manual portfolio, and document mutations are role-gated; briefing/risk/overlay mutations still need route-specific role checks |
...
2. Continue RBAC tests and guards for briefing and overlay/risk mutations.
